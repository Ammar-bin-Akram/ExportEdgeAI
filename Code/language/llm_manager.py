...
class LLMManager:
    ...
    def __init__(self):
        ...
        if provider == "groq":
            # Primary provider: Groq (OpenAI-compatible)
            self.base_url = GROQ_BASE_URL or os.getenv("GROQ_BASE_URL", "https://api.groq.com/openai/v1")
            self.api_key = GROQ_API_KEY or os.getenv("GROQ_API_KEY", "")
            self.model_name = os.getenv("LLM_MODEL_NAME", GROQ_MODEL_NAME)

            if not self.api_key:
                # Keep LM Studio path available as a safe fallback when key is missing.
                self.base_url = LM_STUDIO_BASE_URL or os.getenv("LM_STUDIO_BASE_URL", "http://192.168.0.105:1234/v1")
                self.api_key = "lm-studio"
                self.model_name = os.getenv("LLM_MODEL_NAME", LM_STUDIO_MODEL_NAME)
                self.provider = "lmstudio-fallback"
            else:
                self.provider = "groq"
        else:
            # LM Studio local server (OpenAI-compatible API) kept for compatibility.
            self.base_url = LM_STUDIO_BASE_URL or os.getenv("LM_STUDIO_BASE_URL", "http://192.168.0.105:1234/v1")
        ...
        print(f" LLM Manager initialized — provider={self.provider} @ {self.base_url}")
...
ANSWER:"""
    ...
    def query_llm(self, prompt: str) -> str:
        """Send query to LLM via OpenAI-compatible API (Groq or LM Studio)."""
        try:
