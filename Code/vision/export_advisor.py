...
def parse_structured_recommendation(answer: str) -> Dict[str, list]:
    ...
    Handles various LLM formatting styles:
        - Plain:    RECOMMENDED COUNTRIES:
        - Markdown: **RECOMMENDED COUNTRIES:**
        - Bulleted: - **RECOMMENDED COUNTRIES:**
        - Mixed:    *RECOMMENDED COUNTRIES:**
    ...
    header_map = [
        ...
        (r"conditions(?:\s+before\s+exporting)?", "conditions"),
    ]

    # Build a combined pattern that captures any header
    # Allow optional leading bullets/dashes, markdown bold markers (**)
    combined = "|".join(f"(?P<h{i}>{pat})" for i, (pat, _) in enumerate(header_map))
    header_re = re.compile(
        rf"^\s*[-•*]*\s*\**\s*(?:{combined})\s*\**\s*[:：]",
    ...
    for mi, m in enumerate(matches):
        ...
        for line in block.splitlines():
            line = line.strip()
            # Strip bullet markers, numbering, and markdown bold
            cleaned = re.sub(r"^[-•*]\s*|^\d+[.)\s]+", "", line).strip()
            cleaned = re.sub(r"\*{1,2}(.*?)\*{1,2}", r"\1", cleaned).strip()
            # Remove any remaining stray asterisks
            cleaned = cleaned.strip("* ").strip()
            if cleaned and cleaned.lower() != "none" and len(cleaned) > 1:
                # Skip lines that look like section headers for other sections
                is_header = False
                for pat, _ in header_map:
                    if re.match(rf"\s*\**\s*{pat}\s*\**\s*[:：]", cleaned, re.IGNORECASE):
                        is_header = True
                        break
                if not is_header:
